...
def get_db():
    ...
    # Fix SSL/TLS handshake errors on Python 3.14 / Windows
    try:
        import certifi
        _client = MongoClient(uri, serverSelectionTimeoutMS=10000, tlsCAFile=certifi.where())
    except Exception:
        _client = MongoClient(uri, serverSelectionTimeoutMS=10000, tls=True, tlsAllowInvalidCertificates=True)
    ...
    _db.sessions.create_index("expires_at", expireAfterSeconds=0)  # TTL index
...
def login_user(officer_id: str, password: str):
    """Returns (user_dict, token) or raises ValueError."""
    db   = get_db()
